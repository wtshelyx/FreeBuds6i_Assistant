...
abstract class HeadphonesBase {
  ...
  Future<void> setAncType(HeadphonesAncType type);

  ValueStream<HeadphonesEqMode> get eqMode;

  Future<void> setEqMode(HeadphonesEqMode mode);

  ValueStream<bool> get autoPause;

  Future<void> setAutoPause(bool enabled);

  ValueStream<bool> get dualConnect;

  Future<void> setDualConnect(bool enabled);

  ValueStream<bool> get slide;

  Future<void> setSlide(bool enabled);

  ValueStream<bool> get ldacState;

  Future<void> setLdacState(bool enabled);

  ValueStream<bool> get voiceMode;

  Future<void> setVoiceMode(bool enabled);
  ...
  Future<String> dumpSettings() async => json.encode({
        ...
        'voiceModeState': voiceMode.valueOrNull,
      });
  ...
  Future<void> restoreSettings(String settings) async {
    ...
    for (final i in json.entries) {
      ...
      } else if (i.key == 'voiceModeState') {
        await setVoiceMode(i.value);
      }
    }
  }
}
